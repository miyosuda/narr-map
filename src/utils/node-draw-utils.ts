import { NodeState, NodeDrawState, NodeDrawStateMapType } from '@/types'
...
const USE_OFFSET_Y_FOR_SINGLE_CHILD = true // 子が1ノードしかない場合は少し上に上げるかどうか
...
export function containsPosHalf(
  state: NodeState,
...
const calcDrawInfoMap = (state: NodeState, d: NodeDrawInfoMapType): NodeDrawInfoMapType => {
  ...
  if (state.children.length === 0 || state.folded) {
    ...
    for (let i = 0; i < state.children.length; i++) {
      ...
      const childState = state.children[i]
      const { topY: childTopY, bottomY: childBottomY } = dd[childState.id]
...
const calcXY = (
  ...
  baseY: number,
  width: number,
  drawInfoMap: NodeDrawInfoMapType
) => {
  // baseXにshiftXを足す
...
const calcDrawStateMapSub = (
  state: NodeState,
  rootState: NodeState,
  baseX: number,
  baseY: number,
  d: NodeDrawStateMapType,
  drawInfoMap: NodeDrawInfoMapType
...
const calcChildStartOffsetY = (state: NodeState, drawInfoMap: NodeDrawInfoMapType) => {
  ...
  if (USE_OFFSET_Y_FOR_SINGLE_CHILD && state.children.length === 1) {
...
export function containsPosForHandle(
  state: NodeState,
  px: number,
  py: number,
  drawStateMap: NodeDrawStateMapType
  ...
  if (isRoot(state)) {
    ...
    return (
      px >= handleX - 2 && // 2pxだけ広げてtouchしやすくしている
