...
export function addChildNodeBelow(
  state: NodeState,
  newChildState: NodeState,
...
export function addChildNodeAbove(
  state: NodeState,
  newChildState: NodeState,
  targetNode: NodeState
) {
...
export function findNode(state: NodeState, cond: (state: NodeState) => boolean): NodeState | null {
  ...
  const foundChildren = children
    .map((childState) => findNode(childState, cond))
...
export function updateNodes(
  state: NodeState,
  cond: (state: NodeState) => boolean,
  applyFunc: (state: NodeState) => NodeState
): NodeState {
  if (cond(state)) {
  ...
  if (state.children.length > 0) {
    const newChildren = state.children.map((childState) => updateNodes(childState, cond, applyFunc))
...
export function getSavingNodeState(state: NodeState): SavingNodeState {
  const savingState: SavingNodeState = {
    ...
    accompaniedState:
      state.accompaniedState != null ? getSavingNodeState(state.accompaniedState!) : null
  }
  return savingState
}
...
const getNodeStateFromSavingSub = (savingState: SavingNodeState): NodeState => {
  const state = getNodeState({
    ...
    accompaniedState:
      savingState.accompaniedState != null
...
export function getMaxNodeId(state: NodeState): number {
  ...
  const maxNodeId = extendedChildren
    .map(getMaxNodeId)
    .reduce((maxId, id) => Math.max(maxId, id), state.id)
  return maxNodeId
}
