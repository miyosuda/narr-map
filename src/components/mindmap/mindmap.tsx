...
const getRange = (
  ...
  range: Range=initialRange) : Range => {
...
function MindMap() {
  ...
  function handleMouseDown(e: React.MouseEvent) {
    ...
    if(pickedNodeForHandle != null) {
      ...
      const newRootState = updateNodes(rootState,
      ...
      if(shiftDown) {
        ...
        newRootState = updateNodes(newRootState,
                                   state => state.id === pickedNode!.id,
                                   state => ({
                                     ...state,
                                     selected: true,
      ...
      let newRootState = updateNodes(rootState,
                                     state => state.selected,
                                     state => ({
                                       ...state,
                                       selected: false
  ...
  function handleMouseMove(e: React.MouseEvent) {
    ...
    if(dragState != null) {
      if(dragState.mode === DRAG_NODE) {
        ...
        setCanvasTranslatePos({x: dragState.startElementX + dx,
                               y: dragState.startElementY + dy});
      }            
    } else {
      const pickedNodeForHandle = findNode(rootState, state => containsPosForHandle(state, px, py, drawStateMap));
      if(pickedNodeForHandle != null) {
        const newRootState = updateNodes(rootState,
          state => true,
          state => ({
  ...
  function handleMouseUp(e: MouseEvent) {
    ...
    if(dragState != null) {
      if(dragState.mode === DRAG_NODE) {
        ...
        const ghostTargetState = findNode(rootState, state => state.hoverState !== HOVER_STATE_NONE);
        ...
        if( ghostTargetState !== null) {
          ...
          const targetState = toAccompanied 
            ?
            ghostTargetState.accompaniedState!
            :
            ghostTargetState;
          ...
          if(!hasNodeInAncestor(targetState, newChildState)) {
            ...
            if(targetState.isLeft !== newChildState.isLeft) {
              newChildState = updateNodes(newChildState, 
                state => true,
            ...
            // 移動元の親から外す
            const newRootState0 = updateNodes(newRootState,
            ...
            if(targetHoverState === HOVER_STATE_RIGHT || targetHoverState === HOVER_STATE_LEFT) {
              // 移動先の子として追加
              const newRootState1 = updateNodes(newRootState0,
  ...
  function deleteSelectedNodes() {
    ...
    // selectedのnodeを削除
    const newRootState0 = updateNodes(rootState,
