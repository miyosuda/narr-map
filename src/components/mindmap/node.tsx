...
const calcEdgeOutPos = (state: NodeState, x: number, y: number, width: number, height: number) => {
  if (isRoot(state)) {
    return {
      x: x + width / 2,
      y: y + height / 2
...
const calcEdgeInPos = (state: NodeState, x: number, y: number, width: number, height: number) => {
...
const calcFoldMarkPos = (state: NodeState, x: number, y: number, width: number, height: number) => {
  if (state.isLeft) {
    return {
      x: x,
      y: y + height
...
const Text = (props: TextProps) => {
  ...
  const hoverSelectedStyle = getHoverSelectedStyle(
    props.hoverState,
    props.selected,
    props.isRoot,
    props.darkMode
  ...
  return (
    <foreignObject
      ...
      y={props.y}
    >
      <span className="select-none whitespace-nowrap">{formatEmoji(props.text)}</span>
    </foreignObject>
  )
}
...
export const Node = (props: NodeProps) => {
  ...
  return (
    <>
      ...
      {state.handleShown && <Handle x={handleX} y={handleY} darkMode={darkMode}></Handle>}
    </>
  )
}
